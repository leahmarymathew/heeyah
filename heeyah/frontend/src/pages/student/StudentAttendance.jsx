...
export default function StudentAttendance() {
    ...
    // --- DATA FETCHING EFFECT (ATTENDANCE) ---
    useEffect(() => {
        const fetchAttendance = async () => {
            setLoading(true);
            setAttendance(null);

            // Get the token from localStorage. Your login page must save it here.
    ...
    // --- DATA FETCHING EFFECT (LATEST LEAVE) ---
    useEffect(() => {
        const fetchLatestLeave = async () => {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}/api/leaves/latest`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                    ...
                    <div className="leave-card">
                        ...
                        <p className="leave-status">
                            {latestLeave.approved_by 
                                ? `Approved by ${latestLeave.approved_by}`
                                : 'Pending Approval'
                            }
                        </p>
                    </div>
