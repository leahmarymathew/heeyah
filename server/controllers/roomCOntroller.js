...
// @desc    Get room layout for the allocation page - simple version without heavy auth
// @route   GET /api/rooms/layout/simple
export const getRoomLayoutSimple = async (req, res) => {
    const { floor, search, userRole, rollNo } = req.query;

    try {
        // Basic validation - just check if user info is provided
        if (!userRole || !rollNo) {
            return res.status(400).json({ error: 'User role and ID are required.' });
        }

        let roomQuery = supabase
            .from('room') 
            .select('room_id, room_number, floor, hostel_wing, capacity')
            .eq('floor', floor || '1'); // Default to floor 1
        ...
        if (allocError) {
            ...
            // Continue without allocations data
        }

        const transformedRooms = (rooms || []).map(room => {
            const wing = room.hostel_wing || 'Left'; // Default to Left if not specified
            const roomAllocations = (allocations || []).filter(a => a.room_id === room.room_id);
            const allocationCount = roomAllocations.length;
            const capacity = room.capacity || 3; 
            let status;
        ...
        console.error("Error in getRoomLayoutSimple:", error.message);
        res.status(500).json({ error: 'Failed to get room layout' });
    }
};

// @desc    Get detailed info for one room - simple version without heavy auth
// @route   GET /api/rooms/layout/simple/:id
export const getRoomDetailsSimple = async (req, res) => {
    const { id } = req.params; 
    const { userRole, rollNo } = req.query;
    ...
    try {
        ...
        console.error("Error in getRoomDetailsSimple:", error.message);
        res.status(500).json({ error: 'Failed to get room details' });
    }
};
